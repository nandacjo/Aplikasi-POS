    ...
    <div class="col-lg-12">
        <div class="box">
            <div class="box-header with-border">
                <table>
                    <tr>
                        <td>Supplier {{ $purchase_id ?? 'nihil' }}</td>
                        <td>: {{ $supplier->name }}</td>
                    </tr>
                    <tr>
                        <td>Telepon</td>
                    ...
                    <div class="col-lg-4">
                        <form action="{{ route('pembelian.store') }}" class="form-pembelian" method="post">
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-lg-8">
                                    <input type="number" name="discount" id="discount" class="form-control" value="{{ $diskon }}">
                                </div>
                            </div>
                            <div class="form-group row">
...
let table, table2
     $(function() {
      ...
      $(document).on('input', '.quantity', function() {
        ...
        $.post(`{{ url('/pembelian_detail') }}/${id}`, {
          ...
          .fail(errors => {
            alert('Tidak dapat menyimpan data po');
            return;
          });
      });
    ...
    function loadForm(diskon = 0){
        ...
        $('#total_items').val($('.total_item').text());

         $.get(`{{ url('/pembelian_detail/loadform') }}/${diskon}/${$('.total').text()}`)
        .done(response => {
          $('#totalrp').val(response.total_rp);
          $('#bayarrp').val(response.bayar_rp);
          $('#paid').val(response.price);
          $('.tampil-bayar').text(response.bayar_rp);
        //   $('.tampil-bayar').text('Rp. ' + response.bayar_rp);
